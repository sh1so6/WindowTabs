...
日常開発、build、release は `WindowTabs.sln` と `WindowTabs.CSharp` を中心に進めます。  

## 廃止した構成

- `Win32/` は repo スリム化で廃止・削除した
- `treeviewadv/` は repo スリム化で廃止・削除した

これらは mainline build に含まれない履歴資産だったため、現在は `WindowTabs.sln` と `WindowTabs.CSharp` だけをアクティブな開発レーンとして扱います。
...
提供しているビルドスクリプトを使用して、ポータブル版を自分でビルドすることもできます。
...
任意:
- スタートメニューやデスクトップから起動したい場合は `WindowTabs.exe` のショートカットを作成してください
...
- **タブのコンテキストメニュー**: 右クリックでタブの様々な機能にアクセスできます
  ...
  ---
  - タブのピン止め (サブメニュー: ピン止め/解除、全てピン止め、全て解除、同じ寄せグループ内の左右タブのピン止め/解除)
  - タブの色変更 (サブメニュー: 塗りつぶし / 下線 / 枠線カラーオプション、リセット)
  ---
  - スナップ時のタブマージン (タブグループごとの切り替え)
  - タブ位置 (左寄せ / 右寄せ タブごと、全タブの寄せ変更、グループ内の左右タブの寄せ変更)
  - タブの名前 (変更 / リセット)
  ---
  - 設定
...
トップレベルメニュー:
- 移動 左端 / 移動 右端 - 現在のディスプレイの左端または右端に移動
- 左スナップ / 右スナップ - 画面の左側または右側にスナップ（高さは画面全体）

「他の位置に移動」サブメニュー:
- 移動 上端 / 移動 下端
- 上スナップ / 下スナップ
- 「コーナー」サブメニュー 左上、右上、左下、右下
...
- 日本語の関西弁、東北弁版を同梱
- 言語ファイルでのあらゆる言語をサポート **(WindowTabs.CSharp/Language)**
- 再起動なしで言語を変更可能
- トレイメニューから言語を変更可能
...
- **build_release.bat** - ポータブル ZIP 版をビルド
